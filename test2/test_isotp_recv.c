...
void message_receive( const IsoTpMessage* message ) {
    // You received the message! Do something with it.
  if ( message->completed == true) {
    if ( message->multi_frame == true )
      printf ( "multi-frame message receive success\n" );
    else
       printf ( "single frame message receive success\n" );
  }
  else
...
int main ( void )
{
  ...
  IsoTpReceiveHandle handle;// = isotp_send( &shims, 0x321, 0x123, txBuffer, strlen( txBuffer ), message_sent );

  uint32_t rx_id = 0x123, tx_id = 0x321;
  handle = isotp_receive ( &shims, tx_id, rx_id, message_receive );
#if 0
  uint32_t rxId = 0;
  uint8_t rxLen = 0;
